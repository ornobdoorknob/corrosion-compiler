...
extern cor_void startParser(cor_void);
extern i32 startScanner(ReaderPointer sc_buf);

static cor_void printParserError(string fmt, ...);
static cor_void displayParser(ReaderPointer ptrBuffer);
static i64 getParserFileSize(string fname);
static cor_void callGarbageCollector(cor_void);
...
cor_void printParserError(string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(cor_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
cor_void displayParser(ReaderPointer ptrBuffer) {
...
cor_void callGarbageCollector(cor_void) {
