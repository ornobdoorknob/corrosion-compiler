 ...
 * -------------------------------------------------------------
 */
cor_void bErrorPrint(string fmt, ...);
cor_void displayBuffer(BufferReader* ptr_Buffer);
i64 getFileSize(string fname);
i32 isNumber(const string ns);
cor_void startReader(string, string, cor_char, i32, i32);
...
************************************************************
*/
cor_void startReader(string program, string input, cor_char mode, i32 size, i32 increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	i32 loadSize = 0;		/* the size of the file loaded in the buffer */
	cor_char symbol;			/* symbol read from input file */
